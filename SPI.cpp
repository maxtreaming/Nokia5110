...
SPI::SPI(std::string spi_dev, int SPI_MODE, int LSB_FIRST, int WORD_LEN, int MAX_SPEED)
{
	...
	{
		set_config(SPI_IOC_WR_MODE, SPI_MODE);
		set_config(SPI_IOC_WR_LSB_FIRST, LSB_FIRST);
		set_config(SPI_IOC_WR_BITS_PER_WORD, WORD_LEN);
		set_config(SPI_IOC_WR_MAX_SPEED_HZ, MAX_SPEED);
